...
REF_DIR = ROOT / "assets" / "refs"
OUTPUT_DIR = ROOT / "aether-output"
BACKUP_DIR = ROOT / "assets" / "game-backup"
...
def ref_image_files(file_name: str, no_ref: bool, asset: dict[str, Any] | None = None) -> list[tuple[str, Path]]:
    if no_ref:
        return []
    ref = (asset or {}).get("ref")
    if ref:
        ref_path = Path(ref)
        if not ref_path.is_absolute():
            candidate = REF_DIR / ref
            ref_path = candidate if candidate.exists() else (ROOT / ref)
        if ref_path.exists():
            return [("ref_images", ref_path)]
    path = ASSET_DIR / file_name
    if path.exists():
...
def generate_asset(args: argparse.Namespace, asset: dict[str, Any] | None = None, manifest: dict[str, Any] | None = None) -> Path:
    ...
    file_specs = ref_image_files(file_name, args.no_ref, asset)
...
def cmd_make_sprite(args: argparse.Namespace) -> int:
    ...
    submit_and_download(
        endpoint="/public/v1/sprite/make-sprite/v1",
...
def cmd_process_sheet(args: argparse.Namespace) -> int:
    """Chroma-key, downscale, and re-pack a make-sprite sheet into a game spritesheet."""
    if Image is None:
        raise AetherError("Pillow is required for process-sheet but is not importable.")
    from math import isqrt

    src = Path(args.source)
    if not src.exists():
        raise AetherError(f"Sheet not found: {src}")
    frames = args.frames
    side = isqrt(frames)
    if side * side != frames:
        raise AetherError(f"--frames must be a perfect square (got {frames}).")

    keyed = src
    if not args.no_chroma_key:
        keyed = src.parent / f"{src.stem}-cutout.png"
        remove_chroma_key(src, keyed)

    sheet = Image.open(keyed).convert("RGBA")
    fw, fh = sheet.width // side, sheet.height // side
    nfw, nfh = max(1, round(fw * args.scale)), max(1, round(fh * args.scale))
    if args.padding < 0:
        raise AetherError("--padding must be 0 or greater.")
    if not 0 < args.content_scale <= 1:
        raise AetherError("--content-scale must be greater than 0 and at most 1.")

    content_w = max(1, round(nfw * args.content_scale))
    content_h = max(1, round(nfh * args.content_scale))
    frame_w = nfw + args.padding * 2
    frame_h = nfh + args.padding * 2
    inset_x = args.padding + (nfw - content_w) // 2
    inset_y = args.padding + (nfh - content_h) // 2
    out = Image.new("RGBA", (frame_w * side, frame_h * side), (0, 0, 0, 0))
    for r in range(side):
        for c in range(side):
            cell = sheet.crop((c * fw, r * fh, c * fw + fw, r * fh + fh))
            cell = cell.resize((content_w, content_h), Image.Resampling.LANCZOS)
            out.paste(
                cell,
                (c * frame_w + inset_x, r * frame_h + inset_y),
                cell,
            )

    dest = Path(args.out) if args.out else ASSET_DIR / f"{src.stem}_sheet.png"
    dest.parent.mkdir(parents=True, exist_ok=True)
    out.save(dest)
    print(
        f"Saved {dest} size={out.size[0]}x{out.size[1]} "
        f"frame={frame_w}x{frame_h} content={content_w}x{content_h} "
        f"padding={args.padding} count={frames} grid={side}x{side}"
    )
    return 0


def cmd_prepare_sprite_reference(args: argparse.Namespace) -> int:
    """Shrink a chroma-key character onto a roomy canvas for safer sprite generation."""
    if Image is None:
        raise AetherError("Pillow is required for prepare-sprite-reference but is not importable.")
    if not 0 < args.scale <= 1:
        raise AetherError("--scale must be greater than 0 and at most 1.")

    src = Path(args.source)
    if not src.exists():
        raise AetherError(f"Reference image not found: {src}")
    with Image.open(src) as original:
        image = original.convert("RGBA")
        resized = image.resize(
            (max(1, round(image.width * args.scale)), max(1, round(image.height * args.scale))),
            Image.Resampling.LANCZOS,
        )
        canvas = Image.new("RGBA", image.size, (0, 255, 0, 255))
        free_x = image.width - resized.width
        if args.anchor == "left":
            x = round(free_x * 0.15)
    ...
    print(
        f"Saved {dest} size={canvas.width}x{canvas.height} "
        f"subject={resized.width}x{resized.height} anchor={args.anchor}"
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "process-sheet",
        help="Chroma-key, downscale, and slice a make-sprite sheet into a game spritesheet.",
    )
    p.add_argument("source", help="Raw sprite sheet PNG from make-sprite.")
    p.add_argument("--frames", type=int, default=25, help="Frame count used in make-sprite (perfect square).")
    p.add_argument("--scale", type=float, default=0.25, help="Downscale factor per frame.")
    p.add_argument(
        "--padding",
        type=int,
        default=0,
        help="Transparent padding in output pixels around every frame.",
    )
    p.add_argument(
        "--content-scale",
        type=float,
        default=1.0,
        help="Scale source cell content inside each padded frame (0 < value <= 1).",
    )
    p.add_argument("--out", help="Output spritesheet path. Default assets/game/<name>_sheet.png")
    p.add_argument("--no-chroma-key", action="store_true", help="Skip #00ff00 removal.")
    p.set_defaults(func=cmd_process_sheet)

    p = sub.add_parser(
        "prepare-sprite-reference",
        help="Shrink a character onto the same-size green canvas before make-sprite.",
    )
    p.add_argument("source")
    p.add_argument("--scale", type=float, default=0.65)
    p.add_argument("--anchor", choices=["left", "center", "right"], default="center")
    p.add_argument("--out", required=True)
    p.set_defaults(func=cmd_prepare_sprite_reference)
